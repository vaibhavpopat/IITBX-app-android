...
constructor(version: String) : Comparable<Version> {
    ...
    init {
        val numberStrings = version.split("\\.".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        val versionsCount = Math.min(NUMBERS_COUNT, numberStrings.size)
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return if (other !is Version) false else Arrays.equals(numbers, other.numbers)
    }
    ...
    override fun toString(): String {
        when (numbers.size) {
            0 -> return ""
            1 -> return Integer.toString(numbers[0])
        }
    ...
    fun isNMinorVersionsDiff(otherVersion: Version,
        ...
        return Math.abs(this.majorVersion - otherVersion.majorVersion) >= 1 || Math.abs(this.minorVersion - otherVersion.minorVersion) >= minorVersionsDiff
    }
